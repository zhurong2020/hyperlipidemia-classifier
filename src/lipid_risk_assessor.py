...
def show_primary_prevention_options():
    ...
    # 添加甘油三酯 (TG) 输入
    tk.Label(root, text="甘油三酯 (TG, mmol/L):").grid(row=6, column=0, sticky="w")
    global entry_tg
    entry_tg = tk.Entry(root)
    entry_tg.grid(row=6, column=1)

    tk.Label(root, text="年龄:").grid(row=7, column=0, sticky="w")
    global entry_age
    entry_age = tk.Entry(root)
    entry_age.grid(row=7, column=1)

    # 性别选择
    tk.Label(root, text="性别:").grid(row=8, column=0, sticky="w")
    global gender_var
    gender_var = tk.StringVar(value="male")
    tk.Radiobutton(root, text="男性", variable=gender_var, value="male").grid(row=8, column=1, sticky="w")
    tk.Radiobutton(root, text="女性", variable=gender_var, value="female").grid(row=8, column=2, sticky="w")

    # 第一步高危因素
    tk.Label(root, text="高危筛查:", font=("Arial", 12)).grid(row=9, column=0, columnspan=2, sticky="w")
    global var_diabetes, var_ckd
    var_diabetes = tk.BooleanVar()
    tk.Checkbutton(root, text="糖尿病", variable=var_diabetes).grid(row=10, column=0, sticky="w")
    
    var_ckd = tk.BooleanVar()
    tk.Checkbutton(root, text="CKD 3-4期", variable=var_ckd).grid(row=11, column=0, sticky="w")

    # 第二步危险因素
    tk.Label(root, text="危险因素:", font=("Arial", 12)).grid(row=12, column=0, columnspan=2, sticky="w")
    global var_smoking, var_hypertension
    var_smoking = tk.BooleanVar()
    tk.Checkbutton(root, text="吸烟", variable=var_smoking).grid(row=13, column=0, sticky="w")
    
    var_hypertension = tk.BooleanVar()
    tk.Checkbutton(root, text="高血压", variable=var_hypertension).grid(row=14, column=0, sticky="w")

    # 创建全局变量用于余生危险因素的控件
    global lifetime_risk_frame
    lifetime_risk_frame = tk.Frame(root)
    lifetime_risk_frame.grid(row=15, column=0, columnspan=2, sticky="w")
